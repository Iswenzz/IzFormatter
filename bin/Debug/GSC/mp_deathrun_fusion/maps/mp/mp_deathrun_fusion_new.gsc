...
//AUTO 	//ambientPlay("soundalias");
	...
	//////////dvars/////////////////////////////
//AUTO 	level.dvar["time_limit"]=4.5;
...
//AUTO 	player GiveMaxAmmo( self.pers["m40a3_mp"] );
//AUTO 	player GiveMaxAmmo( self.pers["remington700_mp"] );
...
//AUTO 			level.activ GiveMaxAmmo( self.pers["m40a3_mp"] );
//AUTO 			level.activ GiveMaxAmmo( self.pers["remington700_mp"] );
...
//AUTO 		AmbientPlay("zor");
		...
		wait(15);
			...
			else
			{
				...
				{
							if(players[i]==level.acti)
//AUTO 								players[i] iPrintlnbold( "^1Enemy troops inbound !" );	
							else
//AUTO 								players[i] iPrintlnbold( "Move in !" );
...
//AUTO 		if(level.dvar["time_limit"]<4.5)
//AUTO 		level.dvar["time_limit"]=5;
		
//AUTO 		if(level.dvar["time_limit"]>4.5)
//AUTO 		level.dvar["time_limit"]=5;
		
//AUTO 			iPrintln("^1timer ok");
//AUTO 			for(i=level.dvar["time_limit"];i>0.1;i--)
			{
			if(i>1)
			{
			wait 60;
//AUTO 			iPrintln("^1" + (i - 1) + ":00 ^7remaining");
			if(i==5)
			ent PlaySound( "4mins" );
			if(i==4)
			ent PlaySound( "3mins" );
			...
			{	
			wait 30;
//AUTO 			iPrintln("^1" + (i - 1) + ":30 ^7remaining");
			ent PlaySound( "30secs" );
			wait 15;
//AUTO 			iPrintln("^1" + (i - 1) + ":15 ^7remaining");
			ent PlaySound( "15secs" );
			wait 10;
//AUTO 			iPrintln("^1" + (i - 1) + ":10 ^7remaining");
...
lightningFX()
{
	...
	while(1)
	{
		...
		for (i=0; i<level.players.size; i++)
		{
			if (r==0)
			{
//AUTO 			level playlocalsound("amb_thunder1");
			}
			if (r==1)
			{
//AUTO 			level playlocalsound("amb_thunder2");
			}
		}
...
//AUTO iPrintln(player.name+ " Opened the door for all you slow people");
door delete();
trigger delete();
}
...
/* AUTO trigger_trap waittill("trigger", player);
thread suxtimer();
level.sux=true;
...
//AUTO 			player iPrintlnbold("^1GOTCHA!");
			wait(2);
			player SetOrigin( spot.origin );
			player setplayerangles( spot.angles );
...
{
trigger waittill("trigger", player);
//AUTO iPrintln(player.name+ " finished the secret !");
player SetOrigin( spot.origin );
player setplayerangles( spot.angles );
}
}
...
//AUTO 		player iPrintlnbold("AlphaTeam is full. Wait for a spot");
	wait .5;
}
}
...
//AUTO 		iPrintlnbold(level.alpha_team_counter+ " in alpha team");
//AUTO 		iPrintlnbold(self.name+ " entered AIMGAME!");
...
while(self.onwatch==1)
{
	 if(self.sessionstate != "playing")
		...
		level.alpha_team_counter--;
//AUTO 		iPrintlnbold(level.alpha_team_counter+ " in alpha team");
//AUTO 		iPrintln(self.name +"^1has died. -5 points");
		level.alphascore -= 5;
		self.onwatch = 0;
		}
		else
...
{
trigger waittill("trigger", player);
//AUTO player iPrintln("^1Broke the door !");
wait(0.8);
door delete();
}
...
Xpreward()
{
//AUTO iPrintln(self.name +"^1Found a Booster");
level.alphascore += 5;
}
...
trigger waittill("trigger", player);
	...
	{
//AUTO 	player iPrintln("^Unlocked the door !");
//AUTO 	iPrintlnbold("AlphaTeam has unlocked the next part !");
	wait(0.8);
	door delete();
	locked=false;
	}
	...
	{
//AUTO 	player iPrintlnbold("Your team has not been able to get enough points yet !");
	}
}
}
