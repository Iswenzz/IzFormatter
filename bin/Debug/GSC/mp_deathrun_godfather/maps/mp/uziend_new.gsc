...
AjtoEnd()
{
    while (true)
    {
        ...
        if(level.elsoEndMegvolt == false)
        {
            ...
            level waittill("end_kiv�lasztva", melyik, egyVegy);
...
setend(endname)
{
//Ehhez ne ny�lj, csak a notify a funkci�ja!
    switch(endname)
    {
        case "The Old way":
            level notify("end_kiv�lasztva", "The Old way", false);
        break;
 
        case "Activator need to escape to the safecar":
            level notify("end_kiv�lasztva", "Activator need to escape to the safecar", true);
        break;
 
        case "It's time to rush that house!":
            level notify("end_kiv�lasztva", "It's time to rush that house!", false);
        break;
 
        case "The Don&Challangers get rifles, for the eliminate":
            level notify("end_kiv�lasztva", "The Don&Challangers get rifles, for the eliminate", true);
        break;
         
        case "No weapons, no hopes, only fist":
            level notify("end_kiv�lasztva", "No weapons, no hopes, only fist", true);
        break;
 
        case "Only need perfect jumps...":
        level notify("end_kiv�lasztva", "Only need perfect jumps...", false);
        break;

        case "2 Men 1 roll":
        level notify("end_kiv�lasztva", "2 Men 1 roll", true);
        break;
...
//AUTO         self takeAllWeapons();
//AUTO         self GiveWeapon("m16_mp");
//AUTO         self SwitchToWeapon("m16_mp");
