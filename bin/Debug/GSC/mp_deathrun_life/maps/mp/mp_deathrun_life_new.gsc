...
//AUTO 	level._effect["roomfx"] = loadfx("custome/life_roomfx");
	...
	thread trap11();
	thread trap12();
//AUTO 	thread room_objects();
//AUTO 	thread kniferoom();
//AUTO 	thread sniperroom();
//AUTO 	thread jumproom();
//AUTO 	thread jumproomfail();
//AUTO 	thread jumproomsniper();
...
//AUTO 	iprintlnbold("^5Map Created By VC' Legend");
//AUTO 	iprintlnbold("^3Thanks For Playing");
}
...
opensecret()
{
	...
	trig3 delete();
//AUTO 	player3 iprintlnbold("What now??");
	hidenwall notsolid();
}
...
whendead()
{
	...
	{wait 1;}
	if(self.sc != 0)
//AUTO 	{self.sc = 0;self iprintlnbold(">>Save counter restarted");}
}
...
//AUTO 		player iprintln("^5You Finished Secret and Recived - "+xp+" XP");
		player freezeControls(true);
//AUTO 		wait .01;
		player freezeControls(false);
//AUTO 		wait .1;
...
savetriggers(num)
{
	...
	for(;;)
	{
		...
		{
			player.sc = num;
//AUTO 			player iprintln(">> Position: "+player.sc+" Saved");
		}
		wait .1;
	}
}
...
loadtrigger(num)
{
	...
	for(;;)
	{
		...
		if(num == player.sc)
		{
//AUTO 			player iprintln(">> Position: "+player.sc+" Loaded");
			player setorigin(spot.origin);
			player setplayerangles(spot.angles);
//AUTO 			wait .1;
		}
