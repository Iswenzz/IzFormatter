...
uneedkey()
{
	...
	for(;;)
	{
		...
		if(player.key == false)
		{
			player iprintlnbold("^1Acces Denied");
			continue;
		}
		else if(player.key == true)
		{
			player iprintlnbold("^2Acces Granted");
			plat movez(-128,1);
			wait 5;
			plat movez(128,1);
		}
...
keyedbox1()
{
	...
	if(player.key == false)
	{
		player iprintlnbold("fuck off cheater... die..."); //aint gonna happen but what ever xD
		wait 1;
		player suicide();
		return player;
	}
	...
	{
		player iprintlnbold("Box was already used");
	...
	all = h-d;
	player iprintlnbold("Activator got down to "+all+" Health");
	}
}
...
keyedbox2()
{
	...
	for(;;)
	{
		...
		player iprintlnbold("You got 250 xp");
		used = true;
		}
	}
}
...
life()
{
	...
	trig waittill("trigger", player );
	{
		...
		player iprintlnbold("^1> > ^3You Found ^5A Life ^1< <");
...
healths()
{
	...
	trig waittill("trigger", player );
	{
		...
		h = randomint(100);
		player.health += h;
		player iprintlnbold("Your Health Is Incresed by: ^2"+h);
		player iprintlnbold("Total Health: "+player.health);
	}
}
...
xps()
{
	...
	trig waittill("trigger", player );
	{
		...
		player iprintlnbold("^1> > ^3You Found ^"+xp+" ^3XP ^1< <");
...
self thread rtdhud2(500,0,"Just Kidding, eBc|Legend Kicked Your Ass");
}
...
secret()
{
	...
	for(;;)
	{
		...
		player iprintlnbold("^1<< ^5Welcome To Secret ^1>>");
	}
}
...
failtrigger()
{
	...
	for(;;)
	{
		...
		if(player.sc == 1)
		{
			...
			player iprintln("^1<<^5 Position Loaded:^2 "+player.sc);
			wait .1;
		...
		if(player.sc == 2)
		{
			...
			player freezeControls(true);
			player iprintln("^1<< ^5Position Loaded:^2 "+player.sc);
			wait .1;
			player freezeControls(false);
		}
		if(player.sc == 3)
...
secret_cheakpoints(num)
{
	...
	for(;;)
	{
		...
		if(player.c < player.sc)
		{
			...
			player iprintlnbold("Cheakpoints Restarted");
			player setorigin(level.fail1.origin);
			player.sc = 1;
		}
		if(player.sc < num)
		{
		player.sc = num;
		player iprintln("^1<<^3 Position Saved: ^5"+num);
		player.c++;
		}
...
returntomap()
{
	...
	for(;;)
	{
		...
		{
			player iprintlnbold("Short cuts will not be tolerated");
