...
Unique Roll the Dice for all my maps

The version and functionality may vary as I release new maps (old maps may get the new RTD as v2, v3 release later)!
...
RTDInditasa(trigger)
{
	...
	while (true)
	{
		...
		if (IsDefined(kivalto.voltRTD) == false || kivalto.voltRTD == false)
		{
			...
			kivalto IPrintLnBold("^1Rolling ^2those ^1Dices ^2...");
			kivalto IPrintLnBold("^13");
			wait 1;
			kivalto IPrintLnBold("^32");
			wait 1;
			kivalto IPrintLnBold("^21");
			wait 1;
			HUDTisztito(kivalto, "bold");
			kivalto thread GiftSelector(random);
		}
		else kivalto IPrintLn("^3You cannot use ^1Roll ^2the ^1Dice ^3currently!");
	}
}
...
Headshot(who)
{
	who IPrintLnBold("^2I forgot to take my pills... and everybody hates me...");
    wait 2;
	who IPrintLnBold("^2The only way out... is... a ^9headshot^2!");
    wait 2;
...
DoSuicide(who)
{
	who SayAll("^5I couldn't take it anymore, because I'm too noob for it... I'm sorry.");
    wait 1;
	who Suicide();
}
...
Restart(who)
{
    who IPrintLnBold("^5You were a bad, bad kid... ^8You must restart the map!");
    wait 2;
...
Freeze(who)
{
    who IPrintLnBold("^2Ouch...");
    who IPrintLnBold("^2I forgot to ^3piss ^2before... and it really ^9hurts ^2now!");
    wait 2;
...
Slow(who)
{
	who IPrintLnBold("^5I am a slow, old man... cannot move any faster :(");
    who SetMoveSpeedScale(0.5);
    who waittill("death");
    who SetMoveSpeedScale(GetDVAR("dr_jumpers_speed"));
}

DisallowSprint(who)
{
	who SayTeam("OMG! I CAN NOT RUN?! LOL -.-''");
...
Drunk(who)
{
    ...
    hud_drunkScreen = NewClientHudElem(who);
    hud_drunkScreen.horzalign = "fullscreen";
	hud_drunkScreen.vertalign = "fullscreen";
	hud_drunkScreen.color = (0.7, 0.7, 0.7);
	hud_drunkScreen SetShader( "white", 640, 480 );
	for (i = 0; i <= 0.99; i += 0.01)
	{
		hud_drunkScreen.alpha = i;
		wait 0.2;
	}
    who waittill("death");
    if(IsDefined(hud_drunkScreen)) hud_drunkScreen Destroy();
}

NoAmmo(who)
{
	who IPrintLnBold("^1You have run out of ammo...");
...
Fast(who)
{
	who IPrintLnBold("^2 Yumm, that energy drink was tasty!");
	who IPrintLnBold("^2 I can run significantly faster! *.*");
    who SetMoveSpeedScale(1.5);
    who waittill("death");
    who SetMoveSpeedScale(GetDVAR("dr_jumpers_speed"));
}

//Only part of the script which is not under my copyright: IncreaseJump() method
//Copied from original RTD - all rights go to the original author (should it be BraXi or anyone else)
IncreaseJump(who)
{
	who endon("death");
...
GiveRPG(who)
{
	who IPrintLnBold("^1I stole an RPG-7 from the server admin... haha!");
...
GiveBarrett(who)
{
	who IPrintLnBold("^1I stole a Barrett from the server admin... haha!");
	who GiveWeapon("barrett_mp");
    who GiveStartAmmo("barrett_mp");
    who SwitchToWeapon("barrett_mp");
}

GiveGrenades(who)
{
	who IPrintLnBold("^1I stole some Grenades from the server admin... haha!");
...
GiveHandWeapon(who)
{
    who IPrintLnBold("^1N^2o^3w ^4I ^5a^6m ^7t^8h^9e ^1c^2l^3o^4s^5e^6s^7t ^8t^9h^1i^2n^3g ^4t^5o ^6G^7o^8d^9!");
    who IPrintLnBold("^1A^2n^3d ^4c^5a^6n ^7k^8i^9l^1l ^2w^3i^4t^5h ^6j^7u^8s^9t ^1m^2y ^3h^4a^5n^6d^7s^8.^9.^1.");
	who GiveWeapon("defaultweapon_mp");
    who GiveStartAmmo("defaultweapon_mp");
    who SwitchToWeapon("defaultweapon_mp");
}

PlusLife(who)
{
	who IPrintLnBold("^2Additional Life!");
	who thread braxi\_mod::giveLife();
}

PlayerIsKing(who)
{
	IPrintLnBold("^3THE ^9KING ^3OF THIS SERVER IS ^9" + who.name);
    wait 2;
    IPrintLnBold("^2THE BEST OF THE BESTS^3!!!");
    wait 4;
    IPrintLnBold("/at least for now/");
}

Invisible(who)
{
	IPrintLnBold("^3" + who.name + " ^7is ^9invisible^7 for 30 seconds!");
	who Hide();
	wait 30;
	who Show();
	who IPrintLnBold("^2You are visible again!");
}

AnotherUsePlusGoodGift(who)
{
    who IPrintLnBold("^8You will get a random gift from only the good ones ;)");
    wait 1;
    who IPrintLnBold("^5And then you can use RTD a second time!");
    wait 2;
