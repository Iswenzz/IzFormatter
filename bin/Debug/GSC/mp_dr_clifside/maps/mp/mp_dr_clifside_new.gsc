...
thread Water_Move();
thread rpg();
//AUTO thread give_ammo1();
//AUTO thread give_ammo();
//AUTO thread badmusicv2();
...
//AUTO 	iPrintlnBold( " ^6" + player.name + " ^5Wants to play around in a jugg suit!" ); // announces on screen the player name and the knife room
...
//AUTO 	player giveMaxAmmo("rpg_mp");
	...
	player setPerk("specialty_armorvest");
	player.health = player.maxhealth;
//AUTO 	iPrintlnBold( " ^6" + player.name + " ^1Wants to play around with some heavy weaponry!" ); // announces on screen the player name and the knife room
	if( isDefined( level.activ ) && isAlive( level.activ )) //makes sure the activator exists and is alive, otherwise it'll try to do things to a non-existant acti (._. )
	{
...
//AUTO 		player playLocalSound("waardeloos");
		wait 0.05;
	}
}
