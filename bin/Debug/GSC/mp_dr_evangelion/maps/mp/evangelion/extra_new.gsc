...
AngelAttack()
{
	...
	{
//AUTO 		iprintln("^1[^2World^1]^2:^5Round ^1"+game["roundsplayed"]+"/"+level.dvar["round_limit"]+"^2 Angels Are Removed");
		return;
	}
	...
	{
		case 0:
//AUTO 			iprintlnbold("^1!! Angel 01 Started The Attack !!\n!! Finish The Map Befor The End Base Is Destoryed !! ");
			level.a1=true;
			thread setup();
			break;
...
//AUTO 				iprintlnbold(player.name+" saved the day");
				wait 2;
	...
	if(level.a2 == true)
	{
		...
		trig waittill("trigger",player);
		{
			...
			player.health = 100;
//AUTO 			player iprintlnbold("Health Resotred");
		}
		wait .1;
		}
	}
...
angelstuff()
{
	...
	while(dmg < health)
	{
		...
		if(dmg >= health)
		{
			...
			self delete();
//AUTO 			iprintln("Shots Needed:^2 "+shotsneeded);
//AUTO 			iprintln("Total damage:^2 "+dmg);
//AUTO 			iprintlnbold("Angel Destroyed !!");
		}
	}
}
...
//AUTO 			iprintlnbold(">> Time Runed Out\nNo One Finished <<");
			wait .5;
			level.angelTimer destroy();
		}
...
boxsurprise()
{
	...
	{
		case 0: 
//AUTO 			self braxi\_rank::giveRankXp( undefined, xp );
//AUTO 			iprintln("[^0Box^7]: "+self.name+" Found "+xp+" XP");
			break;
...
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found Desert Eagle");
			break;

		case 2:
			self.health = 200;
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found Health Boost");
			break;

		case 3:
			self freezeControls(true);
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found A Freeze");
			break;

		case 4:
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found Nothing");
			break;

		case 5:
//AUTO 			self setMoveSpeedScale(2);
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found Higher Speed");
			break;

		case 6:
			self thread trail2();
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found A Trail[^01^7]");
			break;

		case 7:
			self thread trail1();
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found A Trail[^02^7]");
			break;
...
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+" ^7Found Nothing");
		...
		case 9:
//AUTO 			iprintln("[^0Box^7]: ^1"+self.name+"^7 Got Disarmed");
...
secret_door()
{
	...
	trig waittill("trigger",p);
	{
		...
		thread secret();
		thread backtomap();
//AUTO 		p iprintlnbold("Secret Door Is Open");
	}
}
...
//AUTO 		player iprintlnbold("Every fail reduses time for 5 secounds!");
		wep = player GetCurrentWeapon();
//AUTO 		player takeallweapons(); //just in case if someone wants to spawn in secret on some other position - inserction
//AUTO 		wait .05;
//AUTO 		player giveweapon(wep);
//AUTO 		player switchtoweapon(wep);
//AUTO 		player givemaxammo(wep);
//AUTO 		wait .05;
	}
}
...
save_loadtrigs(num)
{
	...
	for(;;)
	{
		...
		player.counter = num;
		player.cheaker++;
//AUTO 		player iprintln("^3Position: ^1"+num+" ^3Saved");
		}
...
//AUTO 			player iprintln("^3Position: ^1"+player.counter+" ^3Loaded");
//AUTO 			wait 0.1;
...
//AUTO 		iprintlnbold(player.name+" ^5Finished Secret");
//AUTO 		player braxi\_rank::giveRankXp( undefined, 200 );
//AUTO 		wait .05;
	}
}
...
whenmapends()
{
	...
	players = getentarray("player","classname");
//AUTO 	iprintlnbold("^5* * ^3 Map Created by ^2Legend^5 * *\n* * Thanks on playing * *");
	for(i=0;i<=players.size;i++)
	{
...
//AUTO 	self iprintlnbold("^5* * ^2You Recived Additional ^3"+randomxp+" ^2XP ^5* *");
//AUTO 	self braxi\_rank::giveRankXp( undefined, randomxp );		
}
