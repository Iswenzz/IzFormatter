...
{
    self waittill("trigger",player);
//AUTO     iprintlnbold("^7" + player.name + " has reached the ^1Activator ^7first!");
...
movethespawn()
{
    ...
    self hide();
    wait 5;
//AUTO     iprintlnbold(player.name + " has turned on the UFO.");
...
{
    self waittill("trigger",player);
//AUTO     iprintln(player.name + " tripped a trap for 5 seconds.");
...
afksmasher() //causes the wall near the entrance to move slowly, pushing AFK players off of the first platform.
{
    self waittill("trigger", player);
    self hide();
//AUTO     iprintlnbold("All players on the spawning platform");
//AUTO     iprintlnbold("will be jettisoned in 10 seconds.");
    wait 5;
