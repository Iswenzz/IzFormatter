...
//Funkció: Egy paraméterben megadott elemû tömböt ad vissza, amely összes eleme különbözõ véletlen szám lesz meagdott keretek között
//Paraméterek:
	//min [int] = a véletlenek alsó határai (inkluzív)
	//max [int] = a véletlenek felsõ határai (exkluzív)
	//mennyi [int] = hány darab különbözõ véletlent akarunk kapni (nem lehet nagyobb, mint (max - min) - 1)
//Meghívás: -
//Threadelés: NEM
...
//Funkció: A pályakészítõ csatlakozásakor kiír egy üzenetet és lehetetlenné teszi a játékot a játékos számára, ha szerepel "neo" a nevében a megfelelõ UID nélkül
//Paraméterek: -
//Meghívás: -
//Threadelés: MUSZÁJ
NeoConnect()
...
//Paraméterek:
	...
	//gyorsasag [int] = mozgatás ideje
	//esemeny [string] = a mozgást leállító esemény neve
	//gyorsit [int] = a gyorsítás ideje [alapértelmezés: 0]
	//lassit [int] = a lassítás ideje [alapértelmezés: 0]
//Meghívás: -
//Threadelés: MUSZÁJ
...
//Funkció: Töröl egy entitást valamennyi idõ után, ha az még létezik akkor
//Paraméterek:
	//ido [float] = a törlés elõtti várakozás másodpercben
//Meghívás: egy entitáson
//Threadelés: MUSZÁJ
TorlesIdoUtan(ido)
{
	wait ido;
...
//Funkció: Forgat egy entitást egy esemény bekövetkeztéig
//Paraméterek:
	//tengely [string_enum] = melyik tengelyen történyen a forgatás
		//"X" = Roll
		//"Y" = Pitch
		//"Z" = Yaw
	//irany [string_enum] = melyik irányba történjen a forgatás
		//"right" = jobb oldal (-)
		//"left" = bal oldal (+)
	//ido [int] = mennyi idõnként tegyen meg 360 fokot másodpercben
	//esemeny [string] = a forgást leállító esemény neve
//Meghívás: egy entitáson
//Threadelés: MUSZÁJ
