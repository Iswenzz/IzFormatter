...
//AUTO precacheMenu( "speedex_musicmenu" );
...
//AUTO ambientPlay("ambience");
	...
	preCacheItem("c4_mp");
//AUTO  	precacheItem("ak47_silencer_mp");
...
//AUTO 	player openMenu( "speedex_musicmenu" );
...
musicStuff()
{
	...
	for(;;)
	{
	   ...
	   if(menu == "speedex_musicmenu")
	   {
		    if(response == "song1")
		    {
//AUTO 		    	ambientPlay("sti");
		    	trig delete();
		    }
		    if(response == "song2")
		    {
//AUTO 		    	ambientPlay("today");
		    	trig delete();
		    }
		    if(response == "song3")
		    {
//AUTO 		    	ambientPlay("rustig");
		    	trig delete();
		    }
		    if(response == "song4")
		    {
//AUTO 		    	ambientPlay("toujours");
		    	trig delete();
		    }
		    if(response == "song5")
		    {
//AUTO 		    	ambientPlay("havana");
		    	trig delete();
		    }
	   }
	}
...
/* AUTO 	trap1_trigger waittill("trigger", who);
...
trap9_tele_heen()
{
	...
	while(true)
	{
/* AUTO 		trig_heen waittill ("trigger", who);	
...
trap9_tele_terug()
{
	...
	while(true)
	{
/* AUTO 		trig_terug waittill ("trigger", who);	
...
trap5()
{
	...
	bounce1 triggeroff();
	// Wachten tot op de knop wordt gedrukt
/* AUTO 	trap5_trigger waittill("trigger", who);
...
trap7()
{
	...
	// Wachten tot op de knop wordt gedrukt
/* AUTO 	trap7_trigger waittill("trigger", who);
...
for(;;)
	{
		...
		{
//AUTO 		user GiveWeapon( "m14_gl_mp" );
//AUTO 		user GiveMaxAmmo( "m14_gl_mp" );
//AUTO 		iPrintLnBold(user.name + " ^7is here!");
		wait 3;
		}
			else
			{
...
//AUTO 	iPrintlnBold( " ^6" + player.name + " ^2wants to gut the activator!" ); // announces on screen the player name and the knife room
...
//AUTO 	player giveMaxAmmo("remington700_mp"); 
	...
	//player setPerk("specialty_armorvest");
	//player.health = player.maxhealth;
//AUTO 	iPrintlnBold( " ^6" + player.name + " ^2wants to snipe down the activator!" ); // announces on screen the player name and the knife room
	if( isDefined( level.activ ) && isAlive( level.activ )) //makes sure the activator exists and is alive, otherwise it'll try to do things to a non-existant acti (._. )
	{
...
//AUTO 	player giveMaxAmmo("m1014_grip_mp"); 
	...
	player setPerk("specialty_armorvest");
	player.health = player.maxhealth;
//AUTO 	iPrintlnBold( " ^6" + player.name + " ^2wants to blast the activator's face!" ); // announces on screen the player name and the knife room
	if( isDefined( level.activ ) && isAlive( level.activ )) //makes sure the activator exists and is alive, otherwise it'll try to do things to a non-existant acti (._. )
	{
		level.activ setPlayerangles( actik1.angles );
		level.activ setOrigin( actik1.origin ); //teleports acti
//AUTO 		level.activ TakeAllWeapons(); //takes all weapons from acti
//AUTO 		level.activ GiveWeapon( "m1014_grip_mp", 2 ); //gives acti tomahawk
//AUTO 		wait 0.05;
...
actis1 = getEnt( "sniper_acti", "targetname" ); //the origin the acti will teleport
	...
	player setOrigin( jumps1.origin ); //teleports the jumper
//AUTO 	player TakeAllWeapons(); //takes all weapons from jumper
//AUTO 	player GiveWeapon( "ak47_silencer_mp" ); //jumper weapon 
//AUTO 	wait 0.05;
//AUTO 	player SwitchToWeapon("ak47_silencer_mp");
//AUTO 	player giveMaxAmmo("ak47_silencer_mp"); 
//AUTO 	iPrintlnBold( " ^6" + player.name + " ^2wants to spray the activator!" ); // announces on screen the player name and the knife room
	if( isDefined( level.activ ) && isAlive( level.activ )) //makes sure the activator exists and is alive, otherwise it'll try to do things to a non-existant acti (._. )
	{
		level.activ setPlayerangles( actis1.angles );
		level.activ setOrigin( actis1.origin ); //teleports acti
//AUTO 		level.activ TakeAllWeapons(); //takes all weapons from acti
//AUTO 		level.activ GiveWeapon( "ak47_silencer_mp", 2 ); //gives acti tomahawk
//AUTO 		wait 0.05;
//AUTO 		level.activ SwitchToWeapon( "ak47_silencer_mpp"); //this line means they switch to the weapon
//AUTO 		level.activ giveMaxAmmo("ak47_silencer_mp");
		player freezeControls(true);
		level.activ freezeControls(true);
//AUTO 		wait 1;
//AUTO 		player IPrintLnBold("^1Ready?");
