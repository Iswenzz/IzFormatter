...
oldWay()
{
	...
	iPrintLn("Old way chosen by: ^2" + user.name + "^7.");
...
sniperMinigame()
{
	for(;;)
	{
		...
		iPrintLn("Sniper room chosen by: ^2" + user.name + "^7.");
		user setOrigin(spawn.origin);
...
bounceMinigame()
{
	for(;;)
	{
		...
		level.trig_oneinthechamber delete();
		iPrintLn("Knife room chosen by: ^2" + user.name + "^7.");
		user setOrigin(spawn.origin);
...
weaponMinigame()
{
	for(;;)
	{
		...
		if(getTeamPlayersAlive("allies") == 1)
		{
			...
			user setPlayerAngles(spawn.angles);
			user.health = user.maxHealth;
			iPrintLn("Weapon room chosen by: ^2" + user.name + "^7.");
			user takeAllWeapons();
...
oneInTheChamberMinigame()
{
	for(;;)
	{
		...
		iPrintLn("One In The Chamber room chosen by: ^2" + user.name + "^7.");
		user setOrigin(spawn.origin);
		//user setPlayerAngles((0, 90, 0));
		user setPlayerAngles(spawn.angles);
		user.health = 1;
...
{
	player endon("disconnect"); player waittill("death"); iPrintLn("One In The Chamber room is now open.");
}