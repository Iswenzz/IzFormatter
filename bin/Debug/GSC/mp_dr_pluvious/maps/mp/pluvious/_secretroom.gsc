...
secretRoomTriggers()
{
	...
	trigger1 delete();
	user1 iPrintLn("Secret trigger: 1/3.");
	user1 braxi\_rank::giveRankXP("kill", 50);
	trigger2 waittill("trigger", user2);
	trigger2 delete();
	user2 iPrintLn("Secret trigger: 2/3.");
	user2 braxi\_rank::giveRankXP("kill", 50);
	trigger3 waittill("trigger", user3);
	trigger3 delete();
	user3 iPrintLn("Secret trigger: 3/3.");
	user3 braxi\_rank::giveRankXP("kill", 50);
	trigger4 thread maps\mp\_utility::triggerOn();
	brush1 show();
	brush1 solid();
	iPrintLn("^3" + user3.name + "^7 opened the secret!");
}
...
secretRoomCredits()
{
	for(;;)
	{
		...
		trigger waittill("trigger", user);
		user iPrintLn("Wait 15 seconds to re-use.");
		iPrintLnBold("Thanks to:");
		wait 1;
		iPrintLnBold("^2Callan^7, ^2Eli^7, ^2van van^7, ^2Zantheq^7, ^2solzzz^7, ^2Redemption^7, ^2Zye^7, ^2Spinifex^7 and ^2s8n.");
		wait 1;
		iPrintLnBold("for beta testing!");
		wait 13;
	}
}
...
secretRoomSpeed()
{
	for(;;)
	{
		...
		user iPrintLnBold("Your speed has been doubled for 1 minute.");
	}
}
...
secretRoomSpeedUser()
{
	for(i = 0; i < 60; i++)
	{
		...
		if(i == 59)
		{
			...
			if(isAlive(self))
			{
				self iPrintLnBold("Your speed has been reset.");
			}
		}
	}
}
...
secretRoomEarthquake()
{
	for(;;)
	{
		...
		iPrintLnBold("^2" + user.name + "^7 set off an earthquake!");
		wait 15;
	}
}
...
secretRoomSpam()
{
	for(;;)
	{
		...
		trigger waittill("trigger", user);
		user iPrintLn("Wait 15 seconds to re-use.");
		user sayAll("I am a faggot for spamming :)");
		iPrintLnBold("^2" + user.name + " ^1is better than you.");
		iPrintLnBold("^2" + user.name + " ^2is better than you.");
		iPrintLnBold("^2" + user.name + " ^3is better than you.");
		iPrintLnBold("^2" + user.name + " ^4is better than you.");
		iPrintLnBold("^2" + user.name + " ^5is better than you.");
		wait 8;
		iPrintLnBold("^2" + user.name + " ^6is better than you.");
		iPrintLnBold("^2" + user.name + " ^7is better than you.");
		iPrintLnBold("^2" + user.name + " ^8is better than you.");
		iPrintLnBold("^2" + user.name + " ^9is better than you.");
		iPrintLnBold("^2" + user.name + " ^0is better than you.");
		wait 7;
	}
}