...
run_door1()
{
	...
	door = getEnt( "run_door1", "targetname" );
     	...
     	while(1)
     	{
         	...
         	level.activ SetPlayerAngles( acti.angles );
         	level.activ setOrigin( acti.origin );
//AUTO 	 	iPrintlnBold( " ^2" + player.name + " ^5 has finnished run^1!" );
...
//AUTO 	iPrintlnBold( "^1>> ^5First Place^1 << \n ^2" + user.name + " ");
}  
...
//AUTO 		setDvar(getDvar("mp_dr_tron") + "_allow_vender", 1); // This creates the dvar to enable the vending machines

//AUTO 	if(getDvar(getDvar("mp_dr_tron") + "_health_wait") == "")
//AUTO 		setDvar(getDvar("mp_dr_tron") + "_health_wait", 10); // This creates the dvar to set the time between being able to get health again

//AUTO 	level.allow_vender = int(getDvar(getDvar("mp_dr_tron") + "_allow_vender"));
//AUTO 	level.vender_wait = int(getDvar(getDvar("mp_dr_tron") + "_health_wait"));
...
maxhealth()
{
	...
	for(;;)
	{
		...
		{
//AUTO 			player iPrintlnBold("^2Sorry, fat rats are not allowed on this server");
			continue;
		}

		if(isDefined(player.healthwait))
		{
//AUTO 			player iPrintlnBold(randomMessages[randomInt(randomMessages.size)]); 
			continue;
		}
...
//AUTO 			player iPrintlnBold("^5Yummy^1!");//Change it if you want to.
//AUTO 			player iPrintln("^1Health Restored!");
		}
		else
//AUTO 			player iPrintlnBold("^5You already have full health^1."); //Change it if you want to.
