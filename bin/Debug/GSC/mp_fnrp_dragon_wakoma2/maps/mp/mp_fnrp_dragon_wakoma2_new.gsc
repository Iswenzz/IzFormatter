...
//AUTO 	//ambientPlay("ambient_crossfire");
...
//AUTO   setdvar( "jump_height", "350" );
  setdvar( "bg_falldamagemaxheight", "9999" );
  setdvar( "bg_falldamageminheight", "9998" );
//AUTO   setdvar( "player_sprintspeedscale", "2" );
//AUTO   setdvar( "scr_player_sprinttime", "4" );
...
//AUTO buildWeaponUpgrade("weaponupgrade");
...
//AUTO       mapname = getDvar("mapname");
     
      for (i = 0; i <= 30; i++) {
//AUTO         if (getDvar(mapname+"_guid_"+i) != "" &&
//AUTO             getDvar(mapname+"_name_"+i) != "" &&
//AUTO             getDvar(mapname+"_skin_"+i) != "") {
//AUTO           level.cmap_guids[level.cmap_guids.size] = getDvar(mapname+"_guid_"+i);
//AUTO           level.cmap_names[level.cmap_names.size] = getDvar(mapname+"_name_"+i);
    ...
    {
//AUTO       if (getDvar(getDvar("mapname")+"_defaultvips") == "0") return;
...
//AUTO 	c20dragon_vips = getdvar("c20dragon_vips");
        ...
        if(player isTouching(secret_trig_01) && player useButtonPressed())
			{
				if(isSubStr(c20dragon_vips, toLower(getSubStr(player getGuid(), 24, 32)) ) || isSubStr(player.name, "C20Dragon"))
					{
//AUTO             iprintlnbold ( player.name + " Has ^1Open ^7the ^1VIP ^7Armory!");
    ...
    player_trig_message()
    {
      ...
      wait 1;
//AUTO       self iPrintLnBold("^6FNRP ^7Presents");
      wait 2;
//AUTO       self iPrintLnBold("^6A ^7|FNRP|C20Dragon ^6Studio film");
      wait 2;
//AUTO       self iPrintLnBold("^6FNRP ^7Dragon ^6Wakoma 2");
    ...
    broadcast()
    {
       while(1)
       {
//AUTO                iPrintLn("^6Map Made By ^7|FNRP|C20Dragon!");
               wait 10;
//AUTO                iPrintLn("^3Visit www.fnrp-servers.com");
               wait 10;
//AUTO                iPrintLn("^1There will be BLOOD");
               wait 10;          
       }  
    }
    ...
    player_secret_message()
    {
      ...
      wait 1;      
//AUTO       self iPrintLnBold("^7FOUND ^6SECRET ^7ARMORY ^6ROOM");
      wait 15;
...
needpoints = 10000;        //EDIT ME
        ...
        {
//AUTO             player iPrintlnBold( "You need 10,000 points to buy: Vector" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "skorpion_reflex_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "skorpion_reflex_mp" );
//AUTO player GiveMaxAmmo( "skorpion_reflex_mp" ); 
wait(2); 
} 
}
...
needpoints = 10000;        //EDIT ME
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 10,000 points to buy: SCAR-H" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "uzi_acog_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "uzi_acog_mp" );
//AUTO player GiveMaxAmmo( "uzi_acog_mp" ); 
wait(2); 
} 
}
...
needpoints = 10000;
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 10,000 points to buy: M4" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "m4_reflex_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "m4_reflex_mp" );
//AUTO player GiveMaxAmmo( "m4_reflex_mp" ); 
wait(2); 
} 
}
  ...
  { 
    ...
    while(1) 
      { 
        ...
        if( player.points < needpoints )
          {
//AUTO             player iPrintlnBold( "You need 150,000 points to buy: Minigun" );
            continue;
          }
            player.points -= needpoints;
 
//AUTO             player GiveWeapon ( "saw_acog_mp" );
            weapons = player getWeaponsListPrimaries();
            if( weapons.size >= 3 )
            player takeWeapon( player getCurrentWeapon() );
//AUTO             player switchToWeapon ( "saw_acog_mp" );
//AUTO             player GiveMaxAmmo( "saw_acog_mp" ); 
...
needpoints = 5000;
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 5000 points to buy: HK-21" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "saw_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
...
needpoints = 5000;
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 5000 points to buy: F2000" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "m14_acog_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "m14_acog_mp" );
//AUTO player GiveMaxAmmo( "m14_acog_mp" ); 
wait(2); 
} 
}
...
needpoints = 20000;
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 20,000 points to buy: TYPE 25" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "bo2_type25_gl_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "bo2_type25_gl_mp" );
//AUTO player GiveMaxAmmo( "bo2_type25_gl_mp" ); 
wait(2); 
} 
}
...
needpoints = 20000;
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 20,000 points to buy: HAMR" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "bo2_hamr_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "bo2_hamr_mp" );
//AUTO player GiveMaxAmmo( "bo2_hamr_mp" ); 
wait(2); 
} 
}
...
needpoints = 20000;
        ...
        if( player.points < needpoints )
        {
//AUTO             player iPrintlnBold( "You need 20,000 points to buy: LSAT" );
            continue;
        }
        player.points -= needpoints;
 
//AUTO player GiveWeapon ( "bo2_lsat_acog_mp" );
weapons = player getWeaponsListPrimaries();
if( weapons.size >= 3 )
player takeWeapon( player getCurrentWeapon() );
//AUTO player switchToWeapon ( "bo2_lsat_acog_mp" );
//AUTO player GiveMaxAmmo( "bo2_lsat_acog_mp" ); 
wait(2); 
} 
}
